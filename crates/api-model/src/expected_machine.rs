...
impl From<ExpectedMachine> for rpc::forge::ExpectedMachine {
    fn from(expected_machine: ExpectedMachine) -> Self {
        ...
        rpc::forge::ExpectedMachine {
            ...
            disable_lockdown: expected_machine.data.host_lifecycle_profile.disable_lockdown,
        }
    }
}
...
mod tests {
    ...
    fn host_lifecycle_profile_defaults_when_missing_from_json() {
        ...
        assert_eq!(em.data.host_lifecycle_profile, HostLifecycleProfile::default());
    ...
    fn host_lifecycle_profile_is_empty_when_all_fields_none() {
        ...
        let hlp = HostLifecycleProfile { disable_lockdown: Some(true) };
        assert!(!hlp.is_empty());

        let hlp = HostLifecycleProfile { disable_lockdown: Some(false) };
        assert!(!hlp.is_empty());
    }
}
